...
build-module: setup-nix-merged
	...
	cd $(BUILD_MODULE) && cmake .. $(CMAKE_FLAGS) \
		...
		$(if $(NIX_QTBASE),-DCMAKE_PREFIX_PATH="$(NIX_QT_PREFIX)" -DQT_ADDITIONAL_PACKAGES_PREFIX_PATH="$(NIX_QTDECL);$(NIX_QTREMOBJ)",) \
		&& cmake --build . --target chess_module_plugin -j$$(nproc)
	mkdir -p $(MODULES_DIR)/chess_module
	cp $(BUILD_MODULE)/chess_module_plugin.so $(MODULES_DIR)/chess_module/
	cp metadata.json $(MODULES_DIR)/chess_module/manifest.json
	@echo "chess_module ready at: $(MODULES_DIR)/chess_module/"

## Install chess_module to logos-app modules dir
install-module: build-module
	mkdir -p ~/.local/share/Logos/LogosAppNix/modules/chess_module
	cp $(BUILD_MODULE)/chess_module_plugin.so ~/.local/share/Logos/LogosAppNix/modules/chess_module/
	cp metadata.json ~/.local/share/Logos/LogosAppNix/modules/chess_module/manifest.json
	@echo "chess_module installed to ~/.local/share/Logos/LogosAppNix/modules/chess_module/"
...
build-ui-plugin: setup-nix-merged
	...
	cd $(BUILD_UI_PLUGIN) && cmake .. $(CMAKE_FLAGS) \
		...
		-DLOGOS_LIBLOGOS_ROOT=/tmp/logos-liblogos-merged \
		$(if $(NIX_QTBASE),-DCMAKE_PREFIX_PATH="$(NIX_QT_PREFIX)" -DQT_ADDITIONAL_PACKAGES_PREFIX_PATH="$(NIX_QTDECL);$(NIX_QTREMOBJ)",) \
		&& cmake --build . --target chess_ui -j$$(nproc)
	@echo "chess_ui plugin ready at: $(BUILD_UI_PLUGIN)/libchess_ui.so"

## Install chess_ui plugin to logos-app plugins dir
install: build-ui-plugin
	mkdir -p ~/.local/share/Logos/LogosAppNix/plugins/chess_ui
	cp $(BUILD_UI_PLUGIN)/libchess_ui.so ~/.local/share/Logos/LogosAppNix/plugins/chess_ui/chess_ui.so
	cp ui_metadata.json ~/.local/share/Logos/LogosAppNix/plugins/chess_ui/manifest.json
	@echo "chess_ui installed to ~/.local/share/Logos/LogosAppNix/plugins/chess_ui/chess_ui.so"
...
## Install everything: chess_ui + chess_module + kv_module + delivery_module
